...
import { getIconForSection } from '@/shell/components/MainNav'
    ...
    return React.lazy(async () => {
      try {
        ...
        const ShellWrapper = ({ children }: { children?: React.ReactNode }) => {
          ...
          // Map spec labels to section IDs for icon lookup
          ...
          // Groups: Dashboard | Job Requisitions, Candidates, Scorecards | Team Capacity | ATS Connection
          const separatorAfter = new Set(['Dashboard', 'Scorecards', 'Team Capacity'])

          const flatItems = specNavItems.length > 0
            ? specNavItems.map((item, index) => {
                const labelMatch = item.match(/\*\*([^*]+)\*\*/)
                const label = labelMatch ? labelMatch[1] : item.split('→')[0]?.trim() || `Item ${index + 1}`
                const sectionIdForIcon = labelToSectionId[label] || label.toLowerCase().replace(/\s+/g, '-')
                return {
                  label,
                  href: `/${label.toLowerCase().replace(/\s+/g, '-')}`,
                  isActive: sectionIdForIcon === sectionId,
                  icon: getIconForSection(sectionIdForIcon),
                }
              })
            : [
                { label: 'Dashboard', href: '/', isActive: true, icon: getIconForSection('dashboard') },
                { label: 'Items', href: '/items', icon: getIconForSection('job-requisitions') },
                { label: 'Settings', href: '/settings', icon: getIconForSection('ats-connection') },
              ]

          // Build NavEntry[] with separators between groups
          const navigationItems: Array<{ type: 'item'; item: typeof flatItems[number] } | { type: 'separator' }> = []
          for (const item of flatItems) {
            navigationItems.push({ type: 'item' as const, item })
            if (separatorAfter.has(item.label)) {
              navigationItems.push({ type: 'separator' as const })
            }
          }
